function [H12,H21,F12] = rectify(LabelPoints,FundamentalX)
...
F12 = [ 1.68692594539429e-07  1.02824388365127e-05  -0.00248096379831088
9.04820352576612e-07  -3.57610113135827e-06 0.0312478773559688
-0.000789388401968532 -0.0341302448323640 0.998925950267253 ];
F12 = FundamentalX;

% Matching 2D points hard coded

[x1,x2] = correspondences_F12(LabelPoints);

% Get images

fprintf( 1, 'Getting images...\n' );
myflush();
Img1 = imread( 'IMG_1493.JPG' );
Img2 = imread( 'IMG_1492.JPG' );
...
function myflush
  ...
  % If this is Octave, flush standard output. For Matlab do nothing
...
function [x1,x2] = correspondences_F12(LabelPoints)
...
function myimwrite( fn, Img )

  % imwrite is somewhat different in Octave and Matlab

  if exist('OCTAVE_VERSION')
      imwrite( fn, Img(:,:,1), Img(:,:,2), Img(:,:,3) );
  else
      imwrite( Img, fn );
  end;
